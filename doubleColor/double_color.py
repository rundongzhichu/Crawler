...
for type in types:
    ...
    for i in range(1, int(total_cnt/page_size + 1)):
        ...
        if(content != '' and content != None):
            ...
            res = data['result']
            processSsq(type, res)
            print(f"success process page {page_no} of {int(total_cnt / page_size + 1)}，pagesize：{page_size}， total:{total_cnt}")
        # except Exception as e:
        #     print(f"ERROR: process page {page_no} of {int(total_cnt/page_size + 1)}，pagesize：{page_size}， total:{total_cnt}", e)